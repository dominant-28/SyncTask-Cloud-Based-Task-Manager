...
const Usertable = ({ users }) => {
  const TableHeader = () => {
    return (
      <>
        <thead className="border-b border-[#CD6688] bg-[#272333] dark:border-teal-300">
  ...
  const TableRow = ({ user }) => {
    return (
      <>
        <tr className="border-b border-teal-300 text-white bg-[#272333] hover:bg-gray-400">
          <td className="py-2">
...
export default function Dashboard() {
  ...
  const stats = [
    {
      ...
      icon: <FaNewspaper />,
      bg: "bg-[#1d4ed8]",
      weekTasks: data?.alltasks.filter((task) => {
        return moment(task.createdAt).isAfter(moment().startOf("week"));
      }).length,
    },
    {
      _id: "2",
      ...
      weekTasks: data?.alltasks.filter((task) => {
        return (
          ...
          moment(task.createdAt).isAfter(moment().startOf("week"))
        );
      }).length,
    },
    {
  ...
  const Card = ({ label, count, bg, icon, weekTasks }) => {
    return (
      <div className="w-full border border-teal-300 h-32 bg-[#272333] p-5 shadow-md rounded-md flex items-center justify-between">
        <div className="h-full flex flex-1 flex-col justify-between">
          <p className="text-base text-white">{label}</p>
          <span className="text-2xl text-white font-semibold">{count}</span>
          <span className="text-sm text-white">
            {weekTasks} tasks added this week
          </span>
        </div>
        <div
          className={clsx(
            "w-10 h-10 rounded-full flex text-white items-center justify-center text-white",
            bg
          )}
        >
          {icon}
  ...
  return (
    <div className="h-full py-4 ">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
        {stats.map(({ icon, bg, label, total, weekTasks }, index) => (
          <Card
            key={index}
            icon={icon}
            bg={bg}
      ...
      <div className="w-full  my-16 p-4 rounded shadow-sm">
        <h4 className="text-xl text-white font-semibold py-2">
          Chart by Priority
        </h4>
